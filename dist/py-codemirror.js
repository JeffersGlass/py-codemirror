  ...
  */
  nextOverlapping() {
    for (; ; ) {
      ...
      let str2 = fromCodePoint(next), start = this.bufferStart + this.bufferPos;
...
function charBefore(str2, index) {
  return str2.slice(findClusterBreak(str2, index, false), index);
}
function charAfter(str2, index) {
  return str2.slice(index, findClusterBreak(str2, index));
}
...
var PyCodeMirror = class extends HTMLElement {
  outDiv;
  editor;
  stdout_manager;
  stderr_manager;
  ...
  async execute() {
    ...
    this.worker.sync.write = (str2) => {
      this.outDiv.innerText += str2;
    };
    this.worker.sync.writeErr = (str2) => {
      this.outDiv.innerHTML += `<span style='color:red'>${str2}</span>`;
    };
    this.worker.onerror = ({ error }) => {
      this.outDiv.innerHTML += `<span style='color:red'>${str}</span>`;
      console.log(error);
    };
